...
namespace KeyValues
{
    internal static class StringUtils
    {
        ...
        {
            if (startIndex >= limit)
                throw new KeyValueParseException("Escape sequence missing.", startIndex, 0);
            ...
            switch (s[startIndex++])
            {
                ...
                case 'x':
                    ...
                    break;
                default:
                    throw new KeyValueParseException("Unrecognized escape sequence.", startIndex - 1, 1);
            }
        ...
        {
            if (startIndex == limit)
                throw new KeyValueParseException("hex escape sequence missing.", startIndex, 0);

            var hexResult = CharUtils.CharToHexValue(s[startIndex++]);

            if (hexResult == -1)
                throw new KeyValueParseException("Invalid hex escape sequence.", startIndex - 1, 1);
