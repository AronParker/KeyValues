        ...
        internal int InternalParse(string s, int startIndex, int length)
        {
            ...
            while (i < _limit)
            {
                switch (_s[i])
                {
                    ...
                    case '{':
                        if (!BeginChildren())
                            throw new KeyValueParseException("No parent KeyValue specified to start add children to.", i, 1);

                        i++;
                        break;
                    case '}':
                        if (!EndChildren())
                            throw new KeyValueParseException("No parent KeyValue specified to stop adding children to.", i, 1);
